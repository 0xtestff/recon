...
then
	...
	findomain -t $1 -u test/findomain_subdomains.txt;
		echo "[+] findomain Over";
	...
	FN=$(wc -l < test/wordlist_massdns_subdomains.txt)
	if [ $FN -ge 80000 ];then
		rm test/wordlist_massdns_subdomains.txt
	fi
	...
	cat test/*_subdomains.txt | uniq | sort -u | httprobe -t 15000 -c 10|cut -d "/" -f3|sort -u > test/alive_$1.log
